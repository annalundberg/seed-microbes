...
df <- data.frame("Tissue"=sd$Tissue, "Sample_type"=sd$Sample_type, "Farm"=sd$Farm, "Year"=sd$Year, "Seed_source"=sd$Seed_source, row.names=rownames(sd))
df

df.shared <- merge(df,shared, by="row.names")
colnames(df.shared)[1] <- "SampleID"

a <- aggregate(data=df.shared,SampleID ~ Tissue, FUN="mean")
