...
#### NOTES #####
# Right now, this script requires at least one factor-type variable to be included in the measures of interest to function properly. It would be good to modify it so that it could take only numeric-type variables and still work.
...
generate_distance_data <- function(dist.obj,
  ...
  measures.smpl1s <- smpl.data[as.character(dist.obj$smpl1),
                               measures.of.interest[sapply(smpl.data[, measures.of.interest],
                                                           is.factor)]]
  measures.smpl1s <- as.vector(do.call(paste, list(measures.smpl1s, sep="_")))
  measures.smpl2s <- smpl.data[as.character(dist.obj$smpl2),
                               measures.of.interest[sapply(smpl.data[, measures.of.interest],
                                                           is.factor)]]
  measures.smpl2s <- as.vector(do.call(paste, list(measures.smpl2s, sep="_")))
  ...
  for (measure in measures.of.interest) {
    ...
    if(is.numeric(eval(parse(text=paste("smpl.data$", measure, sep=""))))==FALSE) {
      ...
      average <- paste(measure, ".mean", sep="")
      geo_mean <- paste(measure, ".geo_mean", sep="")
      measure.smpl1s <- smpl.data[as.character(dist.obj$smpl1), measure]
      measure.smpl2s <- smpl.data[as.character(dist.obj$smpl2), measure]
      comp.df <- data.frame("comp"=abs(measure.smpl1s - measure.smpl2s),
                            "avg"=as.vector(
                                apply(data.frame(measure.smpl1s, measure.smpl2s), 1, mean)
                                ),
                            "geo"=as.vector(
                                apply(data.frame(measure.smpl1s, measure.smpl2s), 1,
                                      function(x) prod(x)^(1/length(x)))
                                )
                            )
      names(comp.df) <- c(comparison, average, geo_mean)
      dist.obj.data <- cbind(dist.obj.data, comp.df)
    }
  ...
  for (id.factor in id.factors) {
      ...
      if(is.numeric(eval(parse(text=paste("smpl.data$", id.factor, sep=""))))==FALSE) {
